...
| I-4 | **Customer sync** — RamBase → Mosaik (Harmony) + approval write-back Mosaik → RamBase | INT | 🟡 Partial | flows in [flows-customer-sync.md](flows-customer-sync.md) — needs consolidated spec `docs/customer-sync-spec.md` |
...
| I-6 | **RamBase API integration** — auth model, system credential, scoping, environments | INT | 🟡 Partial | research in [rambase-api-audit.md](superpowers/research/rambase-api-audit.md) — needs consolidated integration spec |
...
|---|---|---|---|---|
| F-1 | **Catalog & product browsing** — listing, product detail, anonymous browsing | FEAT | 🟡 Partial | overview §5.1 — needs own spec `docs/catalog-browsing-spec.md` |
...
| F-7 | **Customer registration & approval** — self-register, link/approve to RamBase customer | FEAT | 🟡 Partial | flows in [flows-customer-sync.md](flows-customer-sync.md) — needs `docs/registration-approval-spec.md` |
...
|---|---|---|---|---|
| A-1 | **Authentication & identity** — AWS Cognito, Pre-Token-Generation Lambda, `rambaseCustomerId` claim, RamBase linking | INT/XC | 🟡 Partial | claim mechanics in [flows-customer-sync.md](flows-customer-sync.md) + Service API §3 — needs own `docs/authentication-spec.md` |
...
| # | Area | Type | Status | Spec |
|---|---|---|---|---|
| X-1 | **Localization & markets** — languages (NO only Phase 1), currency (NOK), translations | XC | ⬜ To write (stub) | [localization-spec.md](localization-spec.md) |
| X-2 | **Environments & deployment** — test / staging-UAT / production, RamBase env mapping | XC | 🟡 Partial | Service API §7 — needs consolidated `docs/environments-spec.md` |
| X-3 | **Observability & monitoring** — OTEL, Grafana, alerting, operational ownership | XC | 🟡 Partial | Service API §7 — needs own spec |
...
8. **X-1 Localization**, **X-5 Security & privacy** — cross-cutting, no spec yet.

Items marked 🟡 **Partial** should be **promoted to standalone specs** so they aren't buried inside the
overview or Service API documents: **I-4, I-6, F-1, F-7, A-1, X-2, X-3.**
